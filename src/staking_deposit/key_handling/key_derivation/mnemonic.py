...
from src.staking_deposit.utils.crypto import (
    SHA256,
    PBKDF2,
)
from src.staking_deposit.utils.file_handling import (
    resource_path,
)
...
def _get_word_list(language: str, path: str) -> Sequence[str]:
    ...
    dirty_list = open(os.path.join(new_path, '%s.txt' % language), encoding='utf-8').readlines()
